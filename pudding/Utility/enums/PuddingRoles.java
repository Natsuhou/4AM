package pudding.Utility.enums;

import net.dv8tion.jda.api.entities.Guild;
import net.dv8tion.jda.api.entities.Member;

import java.util.Objects;

public enum PuddingRoles {
    FOUNDER ("679056812229787699", 10),
    ADMIN ("626855987046776841", 9),
    VIP ("670375280686137373", 8),
    MODERATOR ("627639688592883733", 7),
    HELPER ("627639889432936459", 6),
    PUDDING ("741203611433959456", 5),
    PREMIUM_III ("741205623819206728", 4),
    PREMIUM_II ("630838747222638646", 3),
    PREMIUM_I ("741203629951942737", 2),
    MEMBER ("627639516303327242", 1),
    VERIFIED ("627606457030017026", 1),
    TEMP_MEMBER ("677086321780588554", 0),
    WARNING("745421603319447583", 0);

    private final String rankId;
    private final int rankWeight;

    PuddingRoles(String id, int weight) {
        this.rankId = id;
        this.rankWeight = weight;
    }

    public String getRankId() {
        return rankId;
    }

    public int getRankWeight() {
        return rankWeight;
    }

    public static PuddingRoles getRank(String rankId) {
        for (PuddingRoles puddingRoles : PuddingRoles.values()) {
            if (puddingRoles != null) {
                if (puddingRoles.getRankId().equals(rankId)) {
                    return puddingRoles;
                }
            }
        }
        return null;
    }
    public static void addRole(PuddingRoles puddingRoles, Member member, Guild guild) {
        guild.addRoleToMember(member.getId(), Objects.requireNonNull(guild.getRoleById(puddingRoles.getRankId()))).queue();
    }
    public static void removeRole(PuddingRoles puddingRoles, Member member, Guild guild) {
        guild.removeRoleFromMember(member.getId(), Objects.requireNonNull(guild.getRoleById(puddingRoles.getRankId()))).queue();
    }
}
